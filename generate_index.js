...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .panel-btn { background: var(--gold); color: #080706; border: none; border-radius: 9px; padding: 0 20px; font-weight: 800; font-size: 0.8rem; cursor: pointer; transition: all 0.2s; }
...
<body>
  ...
  <div class="wrap">
      ...
      <div class="body">
        ...
        <div class="step active" id="step-email">
          ...
          <div style="font-size:0.73rem; color:var(--txt3); text-align:center; line-height:1.7;">
            Her IP adresi ve e-postadan haftalık yalnızca 1 başvuru kabul edilmektedir.<br>Devam etmek için Google hesabınızla giriş yapın.
          </div>
        </div>
    ...
    <div class="social-links">
      ...
      <a href="https://music.youtube.com/channel/UCncYK-WS8LG6L52uFAKL53A?si=Uy8vYObZMi02IcPp" target="_blank" class="social-btn" title="YouTube Music" style="font-weight:800; font-size:0.75rem;">YM</a>
      <a href="https://drive.google.com/file/d/1ut-7VB41aSU9p_3Wi5pzNJTEQLgQZNdg/view" target="_blank" class="social-btn" title="Drive Prompts" style="font-weight:800; font-size:0.75rem;">DRV</a>
  ...
  <div class="modal-overlay" id="login-modal">
    <div class="modal-box">
      <div class="modal-header">
        <div class="modal-title">🔑 Yetkili Girişi</div>
    ...
    <div class="modal-box">
      ...
      <div class="modal-body">
        ...
        <div class="tab-pane" id="tab-reviewed">
          <p class="pane-desc">İncelenmiş ve arşive kaldırılmış parçalar. İstediğiniz zaman dinleyebilirsiniz.</p>
          <div class="table-wrap">
            <table class="data-table">
              <thead><tr><th>Parça</th><th>Sanatçı</th><th>Tarih</th></tr></thead>
  ...
  <!-- ═══ PLAYLIST MODAL (Staff Only) ════════════════════════════════════ -->
  <div class="modal-overlay" id="playlist-modal">
    <div class="modal-box">
      <div class="modal-header">
        <div class="modal-title">🎵 Gönderiler Arşivi (İncelenenler)</div>
  ...
  <script>
    ...
    function startGSI() {
      ...
      const iv = setInterval(() => {
        n++;
        if (tryGSI() || n > 80) clearInterval(iv);
      }, 200);
    }
    ...
    async function loadPanelData() {
      try {
        const data = await authFetch('/api/admin/submissions', null);
        panelData = data;
        renderInbox();
        renderReviewed();
        renderLimits();
    ...
    function switchTab(name) {
      document.querySelectorAll('.tab-btn').forEach((b, i) => b.classList.toggle('active', ['inbox','reviewed','limits','accounts'][i] === name));
    ...
    function toggleNote(id) {
      const nb = document.getElementById('note-' + id);
      if (nb.style.display === 'block') nb.style.display = 'none';
      else nb.style.display = 'block';
    }
    ...
    function renderLimits() {
      ...
      const limitMap = {};
      panelData.submissions.forEach(s => {
        const t = new Date(s.timestamp).getTime();
        if (!limitMap[s.email] || limitMap[s.email] < t) limitMap[s.email] = t;
      });
    ...
    async function updateStatus(id, status) {
      try {
        await authFetch('/api/admin/update-status', { id, status });
        loadPanelData();
      } catch(e) { alert(e.message); }
    }

    async function resetLimit(email) {
      if (!confirm(email + ' kullanıcısının bekleme süresi sıfırlansın mı?')) return;
      try { await authFetch('/api/admin/reset-user', { targetEmail: email }); loadPanelData(); }
      catch(e) { alert(e.message); }
    }
